...
export const extractNameFromNote = async (contactString: string) => {
    ...
    const content = `The following snippet contains the information about a person ${contactString}. You have to identify the name of the person and the company they are from. Then extract it and return it as a string of this format: "{NAME}". You have to strictly adhere to this format.
    For Example: if the snippet says - "Had a really interesting conversation with Ankit Sanghvi from Meta Platforms Inc., he's building something with AI" return the string "Ankit Sanghvi". If you don't know the answer, return JUST an EMPTY STRING.`
...
// export const convertContactToString = (contact: Contact): string => {
//     let contactText = "";

//     if (contact.names.length > 0) {
//         contactText += `${contact.names[0].givenName} ${contact.names[0].familyName} `;
//     }

//     if (contact.emailAddresses.length > 0) {
//         contactText += `${contact.emailAddresses[0].value} `;
//     }

//     if (contact.organizations.length > 0) {
//         contactText += `${contact.organizations[0].name} ${contact.organizations[0].title} `;
//     }

//     if (contact.locations.length > 0) {
//         contactText += `${contact.locations[0].value} `;
//     }

//     if (contact.biographies.length > 0) {
//         contactText += `${contact.biographies[0].value} `;
//     }

//     if (contact.interests.length > 0) {
//         const interests = contact.interests.map(interest => interest.value).join(", ");
//         contactText += `Interests - ${interests}`;
//     }

//     if (contact.onlineSearchResults.length > 0) {
//         const searchResults = contact.onlineSearchResults.map(result =>
//             `${result.title}: ${result.snippet}`
//         ).join("; ");
//         contactText += `Online Search Results: ${searchResults}`;
//     }
